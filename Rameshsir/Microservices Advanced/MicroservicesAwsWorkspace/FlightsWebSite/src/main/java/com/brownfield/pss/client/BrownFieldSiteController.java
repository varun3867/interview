...
public class BrownFieldSiteController {
   ...
   public String greetingSubmit(@ModelAttribute UIData uiData, Model model) {
		Flight[] flights = searchClient.postForObject("http://18.191.60.247:8090/search/get", uiData.getSearchQuery(), Flight[].class); 
   ...
   public String ConfirmBooking(@ModelAttribute UIData uiData, Model model) {
		...
		long bookingId =0;
		try { 
			bookingId = bookingClient.postForObject("http://18.220.166.227:8060/booking/create", booking, long.class); 
   ...
   public String searchBookingForm(Model model) {
	...
	public String searchBookingSubmit(@ModelAttribute UIData uiData, Model model) {
		Long id = new Long(uiData.getBookingid());
 		BookingRecord booking = bookingClient.getForObject("http://18.220.166.227:8060/booking/get/"+id, BookingRecord.class);
 		//Added by Ramesh
	...
	public String bookQuery(@PathVariable String flightNumber, 
			...
			long checkinId = checkInClient.postForObject("http://18.220.166.227:8070/checkin/create", checkIn, long.class);
			//Added by Ramesh
